...
async function renderDashboard() {
  const [analytics, refs, reviews] = await Promise.all([
    api('/api/analytics'),
    api('/api/references?liked=false'),
    api('/api/reviews')
  ]);
...
let refView = 'gallery'; // gallery or moodboard
let refDetail = null;
...
async function renderReferences() {
  ...
  main().innerHTML = `
    ...
    <div class="filter-bar">
      <div style="position:relative"><span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--text-dim)">${icons.search}</span><input type="text" placeholder="Search references..." value="${refFilters.search}" oninput="refFilters.search=this.value" onkeydown="if(event.key==='Enter')renderReferences()" style="padding-left:32px;width:220px"></div>
      <div class="filter-pills">
        ...
        ${CATEGORIES.slice(0,6).map(c => `<div class="pill ${refFilters.category===c?'active':''}" onclick="refFilters.category=refFilters.category==='${c}'?'':'${c}';renderReferences()">${c}</div>`).join('')}
      </div>
    ...
    ${refs.length === 0 ? `<div class="empty-state">${icons.upload}<h3>No references yet</h3><p>Start building your inspiration library</p><button class="btn btn-primary" onclick="openAddReference()">${icons.plus} Add Reference</button></div>` :
      refView === 'moodboard' ?
        `<div class="moodboard-grid">${refs.filter(r=>r.liked).map(r => refCard(r, true)).join('') || '<div class="empty-state"><h3>No liked references</h3><p>Like references to build your moodboard</p></div>'}</div>` :
...
function refCard(ref, large) {
  return `<div class="ref-card ${large?'moodboard-card':''}" onclick="showRefDetail('${ref.id}')">
    <div class="ref-preview"><div class="ref-preview-bg" style="background:${styleGradient(ref.style)}"></div><div class="ref-preview-label">${ref.style || 'reference'}</div>
...
async function renderRefDetail() {
  ...
  main().innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
      <div>
        <div class="ref-preview" style="height:280px;border-radius:var(--radius);overflow:hidden;border:1px solid var(--border)"><div class="ref-preview-bg" style="background:${styleGradient(ref.style)}"></div><div class="ref-preview-label">${ref.style}</div></div>
      </div>
      <div>
...
function openAddColor() {
  openModal(`<div class="modal-title">Add Color</div>
    ...
    <div class="form-group"><label class="form-label">Hex</label><input class="full" id="cHex" placeholder="#ec4899"></div>
...
async function saveColor() {
  ...
  ds.colors.push({ name: $('#cName').value, hex: $('#cHex').value, usage: $('#cUsage').value });
...
function openEditColor(idx) {
  ...
  openModal(`<div class="modal-title">Edit Color</div>
    ...
    <div class="form-group"><label class="form-label">Hex</label><input class="full" id="cHex" value="${c.hex}"></div>
...
async function updateColor(idx) {
  const ds = await api('/api/design-system');
  ds.colors[idx] = { name: $('#cName').value, hex: $('#cHex').value, usage: $('#cUsage').value };
  await apiPut('/api/design-system', ds);
...
function extractFromFigma() {
  openModal(`<div class="modal-title">Extract from Figma</div>
    <p style="color:var(--text-muted);margin-bottom:16px">Provide a Figma file URL. The AI will use MCP to extract design tokens. You can also add notes for manual extraction.</p>
    ...
    <div class="form-group"><label class="form-label">Notes (optional)</label><textarea class="full" id="figmaNotes" rows="3" placeholder="e.g. Primary color is #ec4899, font is DM Sans"></textarea></div>
    <div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="doExtract()">Extract</button></div>`);
}

async function doExtract() {
  await apiPost('/api/design-system/extract', { figmaUrl: $('#figmaUrl').value, notes: $('#figmaNotes').value });
  closeModal(); toast('Extraction initiated'); render();
}
  ...
  if (kbTab === 'documents') {
    ...
    $('#kbContent').innerHTML = `
      <div class="dropzone" id="dropzone" onclick="$('#fileInput').click()">
        ...
        <p style="font-size:11px;margin-top:4px">PDF, MD, TXT, HTML supported</p>
      </div>
...
async function saveNewReview() {
  await apiPost('/api/reviews', { title: $('#revTitle').value, type: $('#revType').value, figmaUrl: $('#revFigma').value });
  closeModal(); toast('Review created'); render();
}

function openEditReview(id) {
  const r = state.reviews || [];
  // Simple status/score edit
  openModal(`<div class="modal-title">Edit Review</div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Status</label><select class="full" id="revStatus">${REVIEW_STATUSES.map(s=>`<option>${s}</option>`).join('')}</select></div>
      <div class="form-group"><label class="form-label">Score (1-100)</label><input class="full" id="revScore" type="number" min="1" max="100"></div>
    </div>
...
async function updateReview(id) {
  const data = {};
  if ($('#revStatus').value) data.status = $('#revStatus').value;
...
async function saveNewProject() {
  await apiPost('/api/projects', { name: $('#projName').value, description: $('#projDesc').value, figmaUrl: $('#projFigma').value, notes: $('#projNotes').value });
  closeModal(); toast('Project created'); render();
}

function openEditProject(id) {
  openModal(`<div class="modal-title">Edit Project</div>
    <div class="form-group"><label class="form-label">Status</label><select class="full" id="projStatus">${PROJECT_STATUSES.map(s=>`<option>${s}</option>`).join('')}</select></div>
    <div class="form-group"><label class="form-label">Notes</label><textarea class="full" id="projNotes" rows="3"></textarea></div>
...
async function updateProject(id) {
  const data = { status: $('#projStatus').value };
